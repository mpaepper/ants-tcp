...
class TcpGame(threading.Thread):
    ...
    def run(self):
        ...
        # finally, THE GAME !
        game_result = run_game(self.planetwars, self.bots, self.opts)
        ...
        db.add_game( self.id, self.map_name, self.planetwars.turn, draws,json.dumps(plr) )
        ...
        mins = int(ds / 60)
        secs = ds - mins*60
        log.info("saved game %d : %d turns %dm %2.2fs" % (self.id,self.planetwars.turn,mins,secs) )
